...
(() => {
    ...
    style.textContent = `
        #cards-layer .absolute-card .node-face-avatar {
            position:absolute; left:-20px; top:50%; width:40px; height:40px; box-sizing:border-box;
            overflow:hidden; border:2px solid rgba(255,255,255,.98); border-radius:999px;
            background:#eee9dd; box-shadow:0 3px 9px rgba(52,78,65,.24); z-index:35;
            pointer-events:none; transform:translateY(-50%);
        }
        #cards-layer .absolute-card.graph-root .node-face-avatar {
            left:-22px; top:50%; width:44px; height:44px; transform:translateY(-50%);
        }
        #cards-layer .absolute-card .node-face-avatar img {
            position:absolute; display:block; max-width:none !important; max-height:none !important;
            margin:0 !important; transform:none !important; transform-origin:0 0;
            filter:sepia(.82) saturate(.72) contrast(1.06);
        }
        @media print { #cards-layer .absolute-card .node-face-avatar { display:none !important; } }
    `;
    document.head.appendChild(style);

    const clamp = (value, min, max) => Math.max(min, Math.min(max, value));
    const faceSignature = face => [face.id, face.mediaId, face.x, face.y, face.width, face.height].join('|');
    ...
    function applyCrop(image, face) {
        ...
        const cropSize = Math.min(Math.min(sourceWidth, sourceHeight), Math.max(faceWidth, faceHeight) * 1.38);
    ...
    function apply() {
        for (const card of cardsLayer.querySelectorAll('.absolute-card[data-node-id]')) {
            const face = preferredByPerson.get(card.dataset.nodeId)?.face || null;
