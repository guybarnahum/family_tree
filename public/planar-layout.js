...
// The persistent graph stays in FamilyGraphStore; this stage only orders and compacts the
// currently visible family units and publishes a final planarity validator.
(() => {
    ...
    if (!core || !Store || !Controller) return;
    ...
    function placeMembersOnly() {
        ...
        for (const unit of globalUnits) {
            ...
            unit.members.forEach((member, index) => {
                ...
                if (index < unit.members.length - 1) cursor += unit.memberGaps?.[index] ?? UNIT_GAP;
            });
        }
    }
        ...
        for (let i = 0; i < parentIdsInUnit.length; i++) {
            for (let j = i + 1; j < parentIdsInUnit.length; j++) {
                ...
                if (Number.isFinite(ax) && Number.isFinite(bx)) {
                    ...
                    x = ((leftNode.x + leftNode.cardWidth / 2) + (rightNode.x - rightNode.cardWidth / 2)) / 2;
                }
                const left = unit.centerX - unit.width / 2;
    ...
    function rowsByGeneration() {
        ...
        for (const row of rows.values()) row.sort((a, b) => a.centerX - b.centerX || a.id.localeCompare(b.id));
        return rows;
    }

    function rowIds(row) { return row.map(unit => unit.id); }
    ...
    function sourceFrequency(row, incomingEdges) {
        ...
        for (const edge of incomingEdges) {
            if (targetIds.has(edge.targetUnitId)) freq.set(edge.sourceKey, (freq.get(edge.sourceKey) || 0) + 1);
        }
        return freq;
    }

    function unitSourceKeys(unit, incomingEdges) {
        return [...new Set(incomingEdges.filter(edge => edge.targetUnitId === unit.id).map(edge => edge.sourceKey))];
    }
    ...
    function choosePrimarySource(unit, incomingEdges, frequency, ranks) {
        ...
        return [...keys].sort((a, b) =>
            ...
            (ranks.get(a) ?? Infinity) - (ranks.get(b) ?? Infinity) || a.localeCompare(b)
        )[0];
    }
    ...
    function buildAttachmentBlocks(row, upperRow, incomingEdges) {
        ...
        blocks.sort((a, b) =>
            a.sourceRank - b.sourceRank || oldBlockIndex.get(a.key) - oldBlockIndex.get(b.key) || a.key.localeCompare(b.key)
        );
        return { blocks };
    }

    function flattenBlocks(blocks) { return blocks.flatMap(block => block.units); }
    ...
    function crossingCostForRows(rows, gens, rowIndex, candidateRow, edges) {
        ...
        if (rowIndex > 0) {
            ...
            const layerEdges = edges.filter(edge => edge.parentUnit.gen === gens[rowIndex - 1] && edge.childUnit.gen === gen);
        ...
        if (rowIndex < gens.length - 1) {
            ...
            const layerEdges = edges.filter(edge => edge.parentUnit.gen === gen && edge.childUnit.gen === gens[rowIndex + 1]);
    ...
    function improveBlockOrder(rows, gens, rowIndex, blocks, edges) {
        ...
        const cost = candidateBlocks => crossingCostForRows(rows, gens, rowIndex, flattenBlocks(candidateBlocks), edges);
        return (blocks.length <= EXACT_BLOCK_LIMIT
            ? core.exactBestOrder(blocks, cost, { maxItems: EXACT_BLOCK_LIMIT })
            : core.bestInsertionOrder(blocks, cost, { maxPasses: blocks.length * 2 })).order;
    }
    ...
    function improveUnitsInsideBlocks(rows, gens, rowIndex, blocks, edges) {
        for (const block of blocks) {
            ...
            const cost = candidateUnits => {
                const candidateBlocks = blocks.map(value => value === block ? { ...value, units: candidateUnits } : value);
                return crossingCostForRows(rows, gens, rowIndex, flattenBlocks(candidateBlocks), edges);
            };
            block.units = (block.units.length <= EXACT_BLOCK_LIMIT
                ? core.exactBestOrder(block.units, cost, { maxItems: EXACT_BLOCK_LIMIT })
                : core.bestInsertionOrder(block.units, cost, { maxPasses: block.units.length * 2 })).order;
        }
    }
    ...
    function blockLayout(block) {
        ...
        for (let i = 1; i < units.length; i++) centers.push(centers[i - 1] + unitSeparation(units[i - 1], units[i]));
    ...
    function targetForBlock(block, incomingEdges, outgoingEdges) {
        ...
        const incoming = incomingEdges.filter(edge => ids.has(edge.targetUnitId)).map(edge => edge.sourceX).filter(Number.isFinite);
        if (incoming.length) return core.median(incoming, 0);
        const outgoing = outgoingEdges.filter(edge => ids.has(edge.sourceUnitId)).map(edge => edge.targetX).filter(Number.isFinite);
    ...
    function packBlocks(blocks, targets) {
        ...
        for (let i = 1; i < layouts.length; i++) {
            positions[i] = Math.max(positions[i], positions[i - 1] + layouts[i - 1].width / 2 + BLOCK_GAP + layouts[i].width / 2);
        }
        for (let i = layouts.length - 2; i >= 0; i--) {
            positions[i] = Math.min(positions[i], positions[i + 1] - layouts[i].width / 2 - BLOCK_GAP - layouts[i + 1].width / 2);
        }
        const delta = core.median(targets.map((target, index) => target - positions[index]), 0);
        layouts.forEach((layout, index) => {
            const center = positions[index] + delta;
            for (const unit of layout.units) unit.centerX = center + (layout.offsets.get(unit) || 0);
        });
    }
    ...
    function blocksForPlacement(row, upperRow, edges) {
        ...
        for (const block of blocks) block.units.sort((a, b) => index.get(a.id) - index.get(b.id));
        blocks.sort((a, b) =>
            Math.min(...a.units.map(unit => index.get(unit.id))) - Math.min(...b.units.map(unit => index.get(unit.id)))
        );
        return blocks;
    }
    ...
    function positionRows(rows, gens) {
        for (let pass = 0; pass < POSITION_SWEEPS; pass++) {
            ...
            for (let gi = 0; gi < gens.length; gi++) {
                ...
                packBlocks(blocks, blocks.map(block => targetForBlock(block, incoming, outgoing)));
                placeMembersOnly();
                edges = semanticEdges();
            }
            for (let gi = gens.length - 1; gi >= 0; gi--) {
                ...
                const targets = blocks.map(block => {
                    ...
                    const inXs = incoming.filter(edge => blockIds.has(edge.targetUnitId)).map(edge => edge.sourceX).filter(Number.isFinite);
                    const outXs = outgoing.filter(edge => blockIds.has(edge.sourceUnitId)).map(edge => edge.targetX).filter(Number.isFinite);
    ...
    function layerCrossings(rows, gens, edges) {
        ...
        for (let gi = 0; gi < gens.length - 1; gi++) {
            ...
            const layerEdges = edges.filter(edge => edge.parentUnit.gen === gens[gi] && edge.childUnit.gen === gens[gi + 1]);
    ...
    function routeDiagnostics(edges) {
        ...
        for (const edge of edges) {
            ...
            for (const [nodeId, rect] of cardRects) {
                ...
                if (segments.some(segment => segmentIntersectsRect(segment, rect))) violations.push({ edge, nodeId });
            }
        }
        return violations;
    }
    ...
    function validatePlanarity(rows, gens) {
        ...
        if (crossingCount) console.error('Family layout crossing invariant violated:', crossingCount, crossingLayers);
        if (cardIntersections.length) console.error('Family connector/card intersection invariant violated:', cardIntersections.length, cardIntersections);
    ...
    Controller.registerPrepareStage({ name: 'planar', order: 30, run: prepareTopology });
    Controller.registerLayoutStage({
        name: 'planar',
        order: 30,
        run() {
            lastPlanarResult = graphReady ? planarizeCurrentLayout() : null;
        }
    });
    Controller.registerValidationStage({
        name: 'planar',
        order: 30,
        run() {
            if (lastPlanarResult) validatePlanarity(lastPlanarResult.rows, lastPlanarResult.gens);
        }
    });
    prepareTopology();
})();
