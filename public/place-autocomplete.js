// Optional GeoNames-backed autocomplete for structured place fields.
// Free-form text remains supported; FamilyApi owns browser transport.
(() => {
    ...
    style.textContent = `
        #person-pane .person-pane-place { position:relative; }
        .person-place-suggestions {
            position:absolute; top:calc(100% + 4px); right:-7px; left:22px; z-index:260; overflow:hidden;
            border:1px solid rgba(163,177,138,.34); border-radius:10px; background:rgba(255,255,255,.99);
            box-shadow:0 10px 24px rgba(52,78,65,.14); direction:rtl;
        }
        .person-place-suggestion {
            display:flex; align-items:center; gap:8px; width:100%; border:0;
            border-bottom:1px solid rgba(163,177,138,.16); background:transparent; color:#344e41;
            padding:7px 9px; text-align:right; cursor:pointer;
        }
        .person-place-suggestion:hover,.person-place-suggestion:focus-visible {
            background:rgba(163,177,138,.13); outline:none;
        }
        .person-place-suggestion-flag { flex:0 0 22px; font-size:17px; text-align:center; }
        .person-place-suggestion-label {
            min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; font:500 11px/1.35 Inter,sans-serif;
        }
        .person-place-attribution {
            padding:4px 9px 5px; color:#9aa19a; font:400 8px/1.2 Inter,sans-serif; text-align:left; direction:ltr;
        }
        .person-place-attribution a { color:inherit; text-decoration:none; }
        .person-place-attribution a:hover { text-decoration:underline; }
    `;
    ...
    const placeFlag = editor => editor.parentElement?.querySelector('.person-pane-place-flag') || null;
    ...
    function storedPlace(editor) {
        const person = window.FamilyGraphStore?.person?.(editor.dataset.id) || globalNodeMap?.get?.(editor.dataset.id);
    ...
    function syncAllStoredFlags() {
        pane.querySelectorAll('[data-field="metadata"][data-meta-kind="place"]').forEach(syncStoredFlag);
    }

    function resultPlace(result) {
        const place = { text: String(result?.name || '').trim() };
    ...
    async function requestSuggestions(editor, query) {
        ...
        if (clientCache.has(cacheKey)) return renderSuggestions(editor, clientCache.get(cacheKey));
        ...
        try {
            ...
            const response = await Api.request(`/api/places?${params}`, { signal: controller.signal });
