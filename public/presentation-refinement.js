...
// Presentation owns CSS/card polish only; RenderController owns geometry + viewport commits.
(() => {
    if (window.__familyPresentationRefinementInstalled) return;
    window.__familyPresentationRefinementInstalled = true;
    ...
    style.textContent = `
        ...
        .absolute-card.graph-root h2[data-field="name"] { transform: translateY(0); }
        ...
        @media (max-width: 768px), (hover: none) and (pointer: coarse) {
            ...
            .absolute-card.graph-root h2[data-field="name"] { transform: translateY(0) !important; }
            .absolute-card.graph-root p[data-field="dates"],
            .absolute-card.graph-root p[data-field="description"] {
                opacity: 1 !important;
    ...
    function updateIdleNameShift(card) {
        ...
        const desiredCenter = card.clientHeight / 2;
        card.style.setProperty('--idle-name-shift', `${Math.max(0, Math.min(48, desiredCenter - nameCenter))}px`);
    }
