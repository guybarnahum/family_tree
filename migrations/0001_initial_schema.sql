-- Baseline D1 schema for Family Graph.
-- Runtime request handlers assume these migrations have been applied and never perform
-- schema creation, ALTERs, index setup, or topology backfills.
...
-- Media metadata lives in D1 while original image bytes live in R2. `media_people`
...
-- Face rectangles are normalized to the original image (0..1), so the same record works at
-- any rendered size. person_id is nullable: a face can be marked before it is named.
...
-- One-time non-destructive topology backfill from the original one-parent/one-spouse columns.
